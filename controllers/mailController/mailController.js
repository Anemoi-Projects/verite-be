...
const addToMailList = async (req, res) => {
  try {
    ...
    let check = MailList.findOne({emailID}).lean();
    if(check){
        return res.status(200).json({ success: true, data: "Already subscribed", isSubscribed: true });
    }
     await MailList.create({ emailID })
    return res.status(200).json({ success: true, data: "Successfully added to mail list", isSubscribed: true });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
