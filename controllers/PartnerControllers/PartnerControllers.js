        ...
        if (url) data.url = url
         ...
         if (req.file) {
                    const result = await cloudinary.uploader.upload(req.file.path, {
                        folder: `partners/${name}`, // Store in 'sections' folder
                    });
...
const getPartner = async (req,res) => {
    try {
        ...
        if (id) {
            ...
            if (!partner) return sendErrorResponse(res, 404, `Parter not found for ${id}`);
...
const updatePartner = async (req, res) => {
    try {
        ...
        if (req.file) {
            ...
            const result = await cloudinary.uploader.upload(req.file.path, {
                folder: `partners/${name || partner.name}`
            });
...
const deletePartner = async (req,res) => {
    try {
        ...
        if (!partner) return sendErrorResponse(res, 404, `Partner not found for ${id}`);

        return res.status(200).json({success: true, data: "Partner deleted successfully"});
    } catch (error) {
        console.error(error);
        return res.status(500).json({ success: false, data: "Internal Server Error" });
    }
